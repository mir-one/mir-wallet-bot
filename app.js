var MirAPI = require('./mir-api');
const Mir = MirAPI.create(MirAPI.MAINNET_CONFIG);
...
sendMoneyAmountScene.on('message', (ctx, next) => {
	new Promise (function(resolve, reject) {
		...
		Mir.API.Node.addresses.balance(address).then((balance) => {
			Users.find({telegram_id: botDataFrom.id})
			.exec()
			.then(mongo_result => {
				Mir.API.Node.addresses.balance(mongo_result[0].mir_address).then((mir_result) => {
					amount = Number(amount*100000000);
					let mir_balance = mir_result.balance;
					mir_balance = Number(mir_balance)+Number(0.001);

					if (amount>mir_balance) {
						return ctx.reply('Insufficient funds');
					} 

					const seed = Mir.Seed.fromExistingPhrase(mongo_result[0].mir_phrase);
					...
					const transferData = {
						...
						feeAssetId: 'MIR',
						fee: 100000,
						attachment: 'Mir bot - https://t.me/mir_wallet_bot',
						timestamp: Date.now()
					};

					Mir.API.Node.transactions.broadcast('transfer', transferData, seed.keyPair).then((responseData) => {
						ctx.scene.leave();
...
sendMoneyScene.on('message', (ctx, next) => {
	new Promise (function(resolve, reject) {
		...
		Mir.API.Node.addresses.balance(address).then((balance) => {
			...
			.then(mongo_result => {
				Mir.API.Node.addresses.balance(mongo_result[0].mir_address).then((mir_result) => {
					if (address === mongo_result[0].mir_address) {
...
let users_schema = mongoose.Schema({
    ...
    mir_address: { type: String, required: true },
    mir_phrase: { type: String, required: true },
...
bot.action('select_eng_lang', (ctx, next) => {
	new Promise (function(resolve, reject) {
		...
		let botDataFrom = parseBotDataFrom(ctx);
		const seed = Mir.Seed.create();

		const newUser = new Users({
			_id: new mongoose.Types.ObjectId(),
			telegram_id: Number(botDataFrom.id),
			mir_address: seed.address,
		    mir_phrase: seed.phrase,
		    bot_lang: 'en'
		});
		newUser
		.save()
...
bot.hears('Receive', (ctx, next) => {
	new Promise (function(resolve, reject) {
		...
		.exec()
		.then(mongo_result => {
			return default_response(ctx, `*${mongo_result[0].mir_address}*`, true);
		})
		.catch(mongo_error => {
			bot.telegram.sendMessage(CONFIG.admin_id, 'BOT ERROR!');
			return default_response(ctx, `Bot error`, false);
...
bot.hears('Wallet', (ctx, next) => {
	new Promise (function(resolve, reject) {
		...
		.then(mongo_result => {
			Mir.API.Node.addresses.balance(mongo_result[0].mir_address).then((mir_result) => {
				let mir_balance = Number(mir_result.balance/100000000);
				return ctx.replyWithMarkdown(`*${mir_balance} MIR*`, Markup.inlineKeyboard([
...
bot.hears('Export seed phrase', (ctx, next) => {
	new Promise (function(resolve, reject) {
		...
		.exec()
		.then(mongo_result => {
			return default_response(ctx, `Mnemonic phrase - *${mongo_result[0].mir_phrase}*\n\n*Save and delete this message*`, true);
		})
		.catch(mongo_error => {
			bot.telegram.sendMessage(CONFIG.admin_id, 'BOT ERROR!');
			return default_response(ctx, `Bot error`, false);
